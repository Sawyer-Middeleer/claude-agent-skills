# Knowledge Base

This folder is a personal knowledge base. Read `conventions.md` before creating or editing any note — it is the authority for structure, frontmatter, tags, and links.

## Operating rules

1. **Conventions first.** `conventions.md` governs every write. If an instruction here conflicts with it, conventions win.
2. **Durable or nothing.** Apply the "what belongs" filter before saving; adding nothing is fine when nothing is worth adding.
3. **One concept per file**, in its canonical home. Link, don't duplicate.
4. **Bump `modified:`** on every edit.
5. **Never delete notes** without explicit confirmation — move or archive instead.

## Maintenance skills

If the second-brain plugin is installed: `/second-brain:capturing-knowledge` (save a learning), `/second-brain:organizing-notes` (consistency pass), `/second-brain:processing-inbox` (clear `_inbox/`), `/second-brain:managing-daily-notes` (daily log).
